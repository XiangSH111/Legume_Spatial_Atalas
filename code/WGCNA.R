...
##filter
meanFPKM=0.00001  ####the threshold can be changed---过滤标准，可以修改
n=nrow(datExpr0)
...
pdf(file = "1_sampleClustering.pdf")
par(cex = 0.6)
par(mar = c(0,4,2,0))
...
traitData = read.table("../data/Figure3_data/Figure3A/trait.txt",row.names=1,header=T,comment.char = "",check.names=F)
dim(traitData)
names(traitData)
...
# Re-cluster samples 因为载入了性状数据，则进行重新聚类
sampleTree2 = hclust(dist(datExpr0), method = "average")
# Convert traits to a color representation: white means low, red means high, grey means missing entry
traitColors = numbers2colors(datTraits, signed = FALSE)
# Plot the sample dendrogram and the colors underneath.

pdf(file="2_Sample_dendrogram_and_trait_heatmap.pdf")
...
adjacency = adjacency(datExpr0,type="signed", power = softPower)
...
minModuleSize = 50
 # 最小模块基因数,可修改# Module identification using dynamic tree cut:
...
# Plot the result
#pdf(file="6_Clustering_of_module_eigengenes.pdf",width=15,height=7)
...
#sizeGrWindow(12, 9)
pdf(file="7_merged_dynamic.pdf", width = 12, height = 9)
...
write.table(moduleColors, file = "moduleColors.xls",sep="\t",col.names=F)
...
#sizeGrWindow(10,6)
pdf(file="8_Module-trait_relationships.pdf")
...
for (trait in traitNames){
  ...
  for (module in modNames){
    ...
    if (nrow(geneModuleMembership[moduleGenes,]) > 1){####进行这部分计算必须每个模块内基因数量大于2，由于前面设置了最小数量是30，这里可以不做这个判断，但是grey有可能会出现1个gene,它会导致代码运行的时候中断，故设置这一步
      ...
      #sizeGrWindow(7, 7)
      pdf(file=paste("9_", trait, "_", module,"_Module_membership-vs-gene_significance.pdf",sep=""),width=7,height=7)
      par(mfrow = c(1,1))
...
#sizeGrWindow(9,9)
pdf(file="12_Network_heatmap_plot_all_gene.pdf",width=9, height=9)
...
pdf(file="13_Network_heatmap_plot_selected_genes.pdf",width=9, height=9)
...
sizeGrWindow(5,7.5)
pdf(file="14_Eigengene_dendrogram_and_Eigengene_adjacency_heatmap.pdf", width=5, height=7.5)
par(cex = 0.9)
...
# Plot the dendrogram
#sizeGrWindow(6,6);
pdf(file="15_Eigengene_dendrogram_2.pdf",width=6, height=6)
par(cex = 1.0)
plotEigengeneNetworks(MEs, "Eigengene dendrogram", marDendro = c(0,4,2,0), plotHeatmaps = FALSE)
dev.off()

pdf(file="15_Eigengene_adjacency_heatmap_2.pdf",width=6, height=6)
